    ...
    public class Bundle {
        ...
        public Bundle(string virtualPath, string cdnPath, params IBundleTransform[] transforms) {
            ...
            if (!virtualPath.StartsWith("~/", StringComparison.OrdinalIgnoreCase)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, OptimizationResources.UrlMappings_only_app_relative_url_allowed, virtualPath), "virtualPath");
            }
            if (transforms != null) {
        ...
        public virtual Bundle Include(params string[] virtualPaths) {
            Exception error = Items.Include(virtualPaths);
            if (error != null) {
                throw error;
            }
            InvalidateCacheEntries();
        ...
        public virtual Bundle Include(string virtualPath, params IItemTransform[] transforms) {
            Exception error = Items.IncludePath(virtualPath, transforms);
            if (error != null) {
                throw error;
            }
            InvalidateCacheEntries();
        ...
        internal void InvalidateCacheEntries() {
            if (HttpContext.Current != null && HttpContext.Current.Cache != null) {
                ...
                List<string> copy = new List<string>(_cacheKeys);
                _cacheKeys.Clear();
